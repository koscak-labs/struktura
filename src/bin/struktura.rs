...
fn cmd_generate(args: &[String]) {
    ...
    match target {
        ...
        "rover" => {
            ...
            // Also emit the C wrapper that bridges F Prime calls to rover_flight
...
fn generate_cfs_app_dir(out: &std::path::Path, channels: &[Channel], window: usize, threshold: f64) {
    use struktura::codegen::{generate_cfs_header, generate_cfs_main, generate_cfs_msgids, generate_dfa_core_h};
    ...
    fs::write(src.join("dfa_core.h"), generate_dfa_core_h(window)).unwrap();
...
fn generate_fprime_dir(out: &std::path::Path, channels: &[Channel], window: usize, threshold: f64) {
    use struktura::codegen::{generate_dfa_core_h, generate_fprime_cpp, generate_fprime_fpp, generate_fprime_hpp};

    fs::create_dir_all(out).unwrap();
    fs::write(out.join("dfa_core.h"), generate_dfa_core_h(window)).unwrap();
...
fn generate_ros_dir(out: &std::path::Path, channels: &[Channel], window: usize, threshold: f64) {
    use struktura::codegen::{generate_dfa_core_h, generate_ros_cmake, generate_ros_monitor, generate_ros_package};
...
/// counting) this takes guard from 45 to 55 of 116 windows and from 46 to 54
