...
A dead sensor is quarantined only once the other channels can pin the inconsistency on it specifically — the largest-inconsistency channel that stays inconsistent no matter which single other channel is set aside. Two channels alone, or a coupled pair plus an unrelated third, can disagree without either being isolated this way; that is reported as an ambiguous cross-channel disagreement, and nothing is quarantined. Once a channel is isolated, its value is reconstructed from the others; how good that reconstruction is depends on how tightly the channel is physically coupled to the rest (rover's own relations explain only 0.4-21% of variance, well short of the >0.9 R² a tightly-coupled system like the spacecraft benchmark shows). A permanent environment change is re-learned through a guarded candidate baseline, which is rolled back if the new regime is really a fault. Drift that looks like a regime change is refused.
...
- **Cross-channel parity cannot tell a monotonic trend from a fault.** The parity leg's model of "what a channel should be" is a static linear fit; a channel whose own normal behavior is a one-directional trend (e.g. battery state-of-charge discharging) eventually leaves the range that fit was calibrated on, and the leg reads that as an inconsistency it cannot explain, exactly like a real fault would look. Also, once one channel is quarantined, parity goes fully silent for every other channel rather than running a reduced check against just the survivors — a deliberate, conservative choice (a false alarm from a weak reduced-mode relation was measured, not hypothesized) but a real coverage gap while a fault is already being handled. See the rover example above: `battery_soc`'s row-2595 alarm is its own normal discharge, not the scripted drain 5 rows later.
