...
**Generated DFA matches the Rust library.** Up to 1.8.4, `struktura codegen` (the single-file C99 monitor and the `--cfs`/`--fprime` variants under `codegen`, not `generate`) computed α on its ring buffer in storage order once it had wrapped, and `struktura codegen` and `generate-hybrid` used different box sizes from Rust (α off by up to 0.76 against Rust `dfa()`); that was fixed in 1.8.5, and `generate --rover` was never affected (it links the Rust monitor directly, no generated DFA). But `generate --cfs`/`--fprime`/`--ros` (the directory generators above, `dfa_core.h` and its three callers) kept a second, untested DFA implementation with a box list good only for window 512, a 512-sample cap on top of that, and no reorder into time order, so their α diverged from Rust `dfa()` for any other `--window` (and, independent of window, was wrong once the ring wrapped) -- that is now fixed too: all three include the same tested `dfa_core.h` (`struktura::codegen::generate_dfa_core_h`, box sizes from `dfa_box_sizes`) and reorder the ring first, the way the single-file generators already did. Tests check the generated C/C++ against Rust after every sample (tests/c_monitor_matches_rust.rs, tests/hybrid_c_matches_rust.rs, tests/cli_generators_match_rust.rs). If you generated DFA monitors with `generate --cfs`/`--fprime`/`--ros` before this fix, regenerate them.
