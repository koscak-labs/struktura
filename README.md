...
struktura guard: 3000 samples x 5 channels (motor_current_A, wheel_rpm, imu_accel_g, battery_soc, temp_motor_C), calibrated on 1000 rows
  note: battery_soc drifts -0.0200 per 1000 rows in the calibration (steady trend); guard expects it to continue; extra drift beyond +-0.0000 per 1000 rows alarms; if this drift is itself a fault use --no-trend
  ...
  row   2576  ⚠ battery_soc (1.1x threshold): the signal keeps deviating from what its baseline predicts (a step, a drift, or a change in its pattern)
  ...
  18 faults detected across 2000 samples (0 adaptations, 2 quarantines)
<!-- /example -->
```
...
| **ESA satellites** | ESA-ADB Mission 1 | | | | no α comparison; `guard` on the benchmark's metrics: [docs/scoreboard/esa-adb.md](docs/scoreboard/esa-adb.md) |
...
- **A calibrated channel keeps its own steady drift; a communication link's orbital period still trips the parity leg.** `guard` (and `HybridMonitor::calibrate`) now certifies a per-channel calibration trend — a steady linear drift or a random walk with drift that passes its own R²/half-series gate — and builds it into the residual/CUSUM baseline and the level-shift band, so continuing the SAME drift does not alarm; a channel drifting FASTER than its certified rate, or breaking the opposite way, still does (`--no-trend` turns this off; `struktura guard` prints the certified rate and the band it will tolerate). Measured on the simulated rover (`cargo run --release --example parity_eval`: 60 seeds, calibration on 1,000 steps): clean-run false alarms fell from 239 to 71 (the battery-voltage level shifts from its calibrated discharge are gone; `battery_soc` — an exact linear discharge in `examples/rover.csv` — is itself certified), battery-fault detection stayed 9/9 (median delay rose from 85 to 122 samples, since master's 85 was partly the clean discharge itself pushing the level leg early) and detection overall stayed 51/60. Every clean run still has a healthy sensor (comm, occasionally battery_soc) quarantined by the cross-channel parity leg, about 120 steps after calibration — parity is unchanged in this release (its own fix is separate future work) — and 60/60 clean runs still show a false event. The certification has no horizon by design, so an estimation error in a certified rate can eventually cross a leg's fixed threshold in a very long run; recalibrate periodically (AutoPilot's guarded self-recalibration does this on its own). A channel whose drift is itself the fault still gets flagged: the calibration self-check always evaluates with `--no-trend`'s semantics, so a drifting calibration keeps its "may contain a fault" warning. A periodic driver (e.g. a signal that swings with the rover's orbit) needs a calibration covering at least one period to certify correctly — under that, expect alarms from it.
